...
_STYLE_RULES = (
    ...
    "  - Once source meaning, mechanics, and glossary terms are locked, "
    "ignore English syntax when needed for natural Russian rhythm: split for "
    "hard stops, vary flat openings, and rebuild cadence without adding facts.\n"
    "  - Resolve short fragments, pronouns, ellipses, and phrases such as "
    "'But only a few' from nearby context before translating them.\n"
    "  - Choose party/team nouns by scene function, not literal English labels: "
    "diplomatic shore contact → 'передовой отряд' or 'отряд на берег'; "
