"""Tests for the S5U-699 presentation-mode / block-payload consistency guard.

Split from ``test_export_validation.py`` to keep both files under the
400-line file-length ceiling (S5U-663). The production code under test
lives in ``scripts/_export_validation.py::validate_presentation_mode_payload_consistency``.
"""
...
class TestValidatePresentationModePayloadConsistency:
    ...
    def test_falsy_non_list_blocks_is_surfaced(self, tmp_path: Path) -> None:
        """S5U-699 Codex review #2 — `blocks == ""` / `blocks == 0` must fail the guard.

        Previously the ``blocks = page_data.get("blocks") or []`` fallback
        silently treated every falsy value as "no blocks" — so a facsimile
        page with ``"blocks": ""`` would pass the guard despite violating
        the schema. Validator now rejects any non-list `blocks` value.
        """
        from _export_validation import validate_presentation_mode_payload_consistency
        ...
        data_dir.mkdir()
        payload = {
            "page": {"id": "p0501", "title": "corrupt"},
            "presentation_mode": "facsimile",
            "blocks": "",
            "figures": {},
        }
        (data_dir / "render_page.p0501.json").write_text(json.dumps(payload))
        errors = validate_presentation_mode_payload_consistency(data_dir, [])
        assert len(errors) == 1
        assert "not a list" in errors[0]
        assert "got str" in errors[0]

    def test_non_object_root_is_surfaced(self, tmp_path: Path) -> None:
        """S5U-699 Codex review #2 — a non-dict root must fail, not AttributeError.

        A render_page payload whose JSON root is a list or string (schema
        boundary corruption) previously crashed the guard on
        ``page_data.get()``. Validator now rejects non-object roots
        explicitly so the publish step fails with a clear message.
        """
        from _export_validation import validate_presentation_mode_payload_consistency

        data_dir = tmp_path / "data"
        data_dir.mkdir()
        (data_dir / "render_page.p0502.json").write_text(json.dumps([]))
        (data_dir / "render_page.p0503.json").write_text(json.dumps("raw-string"))
        errors = validate_presentation_mode_payload_consistency(data_dir, [])
        assert len(errors) == 2
        assert any("p0502" in e and "object-shaped" in e and "got list" in e for e in errors)
        assert any("p0503" in e and "object-shaped" in e and "got str" in e for e in errors)
