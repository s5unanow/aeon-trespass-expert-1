"""S5U-1227 — StructureStage cache-hit replay regression.

Per ``.claude/rules/pipeline.md`` § "Stage-output cache invalidation": when a
stage's summary changes observable shape (here ``StructureResult`` gains the
content-bearing ``page_refs`` field, replacing the count-only summary) the
stage's ``version`` field MUST be bumped (done: 1.6 -> 1.7) **AND** a regression
test must exercise the executor's cache-hit path to assert the side-effect is
preserved on cached runs.

This test runs ``StructureStage`` twice via the executor and asserts every
...
* ``structure`` — the executor-written summary artifact (``StructureResult``),
  which now carries ``page_refs``.
* ``page_ir.v1.en`` — the EN IR, the per-page artifacts ``page_refs`` indexes.
* ``resolved_page.v1`` — the per-page resolved-page side-effect.
"""
...
def _make_ctx(tmp_path: Path) -> StageContext:
    ...
    start_run(
        conn,
        run_id="structure_cache_hit_run",
...
def _run_prerequisites_excluding_structure(ctx: StageContext) -> None:
    """ingest -> extract_native -> symbols (NOT structure)."""
...
def test_structure_stage_cache_hit_preserves_artifacts(tmp_path: Path) -> None:
    """Second ``execute_stage(StructureStage(), ctx)`` must hit the cache and
    leave every first-run artifact on disk, byte-identical.
    ...
    _run_prerequisites_excluding_structure(ctx)
