...
def _heading(bid: str, x0: float, y0: float, x1: float, y1: float) -> HeadingBlock:
    return HeadingBlock(
        block_id=bid, bbox=_rect(x0, y0, x1, y1), level=1, children=[TextInline(text="H")]
    )
...
class TestAssignBlocksToRegions:
    def test_block_inside_region(self) -> None:
        ...
        assert _assign_blocks_to_regions(blocks, regions) == {"b1": "r001"}

    def test_block_outside_all_regions(self) -> None:
        blocks = [_para("b1", 50, 100, 300, 120)]
        regions = [_region("r001", RegionKind.BODY, 400, 400, 500, 500)]
        assert _assign_blocks_to_regions(blocks, regions) == {}
    ...
    def test_multiple_blocks_different_regions(self) -> None:
        blocks = [_para("b1", 50, 100, 200, 120), _para("b2", 350, 100, 500, 120)]
        regions = [
            _region("r001", RegionKind.BODY, 40, 90, 210, 130),
            _region("r002", RegionKind.SIDEBAR, 340, 90, 510, 130),
        ]
        assert _assign_blocks_to_regions(blocks, regions) == {"b1": "r001", "b2": "r002"}
...
class TestDetectCaptions:
    ...
    def test_caption_beyond_threshold(self) -> None:
        ...
        cfg = StructureConfig(caption_proximity_pt=25.0)
        blocks, edges = _detect_captions([fig, para], {}, cfg)
        assert not [b for b in blocks if isinstance(b, CaptionBlock)]
        assert len(edges) == 0
    ...
    def test_text_length_limit(self) -> None:
        fig = _figure("b1", 50, 100, 400, 300)
        para = _para("b2", 50, 310, 400, 325, text="A" * 250)
    ...
    def test_no_figures(self) -> None:
        ...
        blocks, edges = _detect_captions([para], {}, StructureConfig())
        assert blocks == [para]
        assert edges == []
...
class TestPromoteCallouts:
    ...
    def test_multi_block_callout_merge(self) -> None:
        ...
        blocks, _edges = _promote_callouts([p1, p2], {"b1": "r001", "b2": "r001"}, regions)
        callouts = [b for b in blocks if isinstance(b, CalloutBlock)]
        assert len(callouts) == 1
...
class TestResolveTables:
    ...
    def test_without_evidence_unchanged(self) -> None:
        para = _para("b1", 50, 100, 400, 200)
        blocks, edges = _resolve_tables([para], None, StructureConfig())
        assert blocks == [para]
        assert edges == []
    ...
    def test_low_confidence_skipped(self) -> None:
        ...
        blocks, _edges = _resolve_tables(
            [para], _evidence([tc]), StructureConfig(table_min_confidence=0.6)
        )
        assert not [b for b in blocks if isinstance(b, TableBlock)]
    ...
    def test_figure_not_promoted_to_table(self) -> None:
        fig = _figure("b1", 50, 100, 400, 300)
        tc = _table_candidate("e.tbl.001", 45, 95, 405, 305, confidence=0.9)
        blocks, _edges = _resolve_tables(
            [fig], _evidence([tc]), StructureConfig(table_min_confidence=0.6)
        )
...
class TestBboxOverlap:
    def test_full_overlap(self) -> None:
        assert _bbox_overlap(_rect(0, 0, 100, 100), _rect(0, 0, 100, 100)) == 1.0

    def test_no_overlap(self) -> None:
        assert _bbox_overlap(_rect(0, 0, 50, 50), _rect(100, 100, 200, 200)) == 0.0

    def test_partial_overlap(self) -> None:
        overlap = _bbox_overlap(_rect(0, 0, 100, 100), _rect(50, 50, 150, 150))
        assert 0.0 < overlap < 1.0


class TestBuildAnchorEdges:
    def test_correct_edge_kinds(self) -> None:
        edges = _build_region_edges([_para("b1", 50, 100, 200, 120)], {"b1": "r001"})
        assert len(edges) == 1
    ...
    def test_unmapped_blocks_no_edges(self) -> None:
        assert _build_region_edges([_para("b1", 50, 100, 200, 120)], {}) == []
...
class TestBuildResolvedBlocks:
    def test_resolved_block_types(self) -> None:
        ...
        resolved = _build_resolved_blocks(blocks, {"b1": "r001", "b2": "r002"})
...
class TestResolveSemanticsIntegration:
    def test_end_to_end(self) -> None:
        ...
        result = resolve_semantics(
            ...
            regions,
            None,
            StructureConfig(caption_proximity_pt=25.0),
        )
        assert isinstance(result, SemanticResolution)
        assert len(result.blocks) == 4
        assert len([b for b in result.blocks if isinstance(b, CaptionBlock)]) == 1
        assert len([b for b in result.blocks if isinstance(b, CalloutBlock)]) == 1
