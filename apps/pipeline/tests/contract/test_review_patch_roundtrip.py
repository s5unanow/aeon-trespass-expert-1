...
def test_review_export_applies_and_revalidates_render_page(tmp_path: Path) -> None:
    ...
    target_path = tmp_path / target_ref.relative_path
    target_path.parent.mkdir(parents=True)
    target_path.write_text(json.dumps(render_data), encoding="utf-8")
    ingestible_render = ArtifactStore(tmp_path).get_json(target_ref)
