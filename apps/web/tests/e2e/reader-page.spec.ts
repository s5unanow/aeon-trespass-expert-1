...
test('walking skeleton reader page renders correctly', async ({ page }) => {
  ...
  // Check icon is rendered with correct aria-label
  const icon = page.locator('[role="img"][aria-label="Прогресс"]');
  await expect(icon).toBeVisible();
  await expect(icon).toHaveAttribute('data-symbol-id', 'sym.progress');
