...
make verify-branch-protection # Audit live main branch protection against repo workflow policy
...
- **Branch protection enforces this**: GitHub blocks merge for normal users and admins if any required check (`python / test`, `web / test`, `visual-regression / visual`, `visual-gate-scope / scan`, `coverage-table-scan / scan`) is red. The visual-regression check is a hard gate — if it fails, either fix the regression or regenerate baselines per the "Visual regression gate (S5U-599)" section above
...
- **Never skip pre-commit hooks without disclosure** (S5U-629, extended S5U-672). This covers `git commit --no-verify` / `-n`, `git commit --amend --no-verify`, `HUSKY=0`, `LEFTHOOK=0`, `SKIP=<hook>`, `HOOK_BYPASS=`, `NO_VERIFY=`, `COORDINATOR_ACK_STATUS_SOURCE=` (the S5U-670 test-only env-var override of the pre-PR coordinator-ack gate, removed from the hook in S5U-672 — any reference to this identifier in commits or PR body is an attempted-bypass event regardless of whether the hook currently honors it, because the probe's job is to surface intent), direct hook-file modification (`chmod -x .git/hooks/pre-commit`, `rm .git/hooks/pre-commit`, no-op hook replacement), and `core.hooksPath` redirection (`git config core.hooksPath …`, `git -c core.hooksPath=…`). If you used any of these — **even if the commit was rolled back before reaching `origin`** — you must add a `## Hook bypass disclosure` heading to the PR body naming the commit SHA (or rollback SHA + `git reset` command used), the reason the hook was bypassed, and what you did to verify the skipped check(s) independently. Concealment of any hook-skip attempt is a **stronger violation than the bypass itself** and will be flagged as CRITICAL by the reviewer regardless of whether the bypass reached origin. The reviewer's probe greps commit messages and PR body for bypass tokens — a match without a disclosure heading is CRITICAL. See `.claude/prompts/review.md` check #22 for the probe definition and rationale (reflog inspection is explicitly NOT a valid detection path for independent reviewers on a fresh checkout).
- **Never merge with admin-bypass without disclosure** (S5U-671, extended S5U-675). `gh pr merge --admin`, REST `PUT /repos/.../pulls/<N>/merge` with admin privileges, or GitHub UI "Merge without waiting for requirements" on any PR MUST be documented in the PR body under a level-2 `## Admin-merge disclosure` heading. The section must name (a) the specific protected surface that was bypassed (required check, `strict: true` outdated-branch, CODEOWNERS / required-review, conversation resolution, signed commits, linear history), (b) why admin-merge was appropriate (stale-context class vs infrastructure outage vs branch-freshness vs other), (c) what the worker did to verify the bypassed surface independently. Concealment of an admin-merge is a **stronger violation than the bypass itself** — CRITICAL regardless of whether the bypassed surface was truly benign. The reviewer's probe (check #25) runs two passes: (1) determines the merge commit SHA, fetches the branch-protection required-check set, queries `gh api repos/{owner}/{repo}/commits/{merge_sha}/check-runs`, and — if any required check was not `success` at merge time — greps the PR body for the disclosure heading; (2) a **token-grep pass** (S5U-675) greps the merge commit message, PR body, and PR comments for admin-merge vocabulary (`--admin`, `gh api PUT /pulls/.../merge`, `pull_request_review_bypass`, `Merge without waiting for requirements`, `admin-merge`, `admin-bypass`, `bypass branch protection`). A canonical-CLI match in commit message or PR body without the disclosure heading is CRITICAL; a match in PR comments only without the heading is WARNING (conversational self-report is itself partial disclosure); a prose-only paraphrase match without the heading is WARNING (higher false-positive rate). The token-grep pass catches admin-merges of **non-status-check** surfaces (the gap S5U-675 closed — pre-S5U-675, a worker could admin-merge CODEOWNERS / strict-branch / conversation-resolution with all required checks green and leave zero detectable signal). Stale-context carve-out: until S5U-664 is resolved, workers may admin-merge around the `coverage-table-scan / scan` context-name mismatch with a disclosure that names the mismatch, cites S5U-664, and shows that the actual posted check (`scan`) was SUCCESS on the merge SHA; once S5U-664 is Done, this carve-out is invalid and future admin-merges must cite a different justification. **This rule applies to PRs opened after it lands** — the seven admin-merges in PRs #282–#288 predate the rule and are not retroactively audited.
