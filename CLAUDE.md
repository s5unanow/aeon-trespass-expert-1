...
- **Adding a new top-level quality gate**: any new required-check-style job (Python/JS scanner, YAML linter, etc.) MUST be added to branch protection's `required_status_checks.contexts` in the same PR that introduces the workflow. Use `gh api -X PATCH repos/{owner}/{repo}/branches/main/protection/required_status_checks -f contexts[]=<name>`. This was the latent gap in S5U-608 that S5U-611 closed — a job wired into `ci.yml` but absent from `contexts` is one-diff-away from being silently removed.
