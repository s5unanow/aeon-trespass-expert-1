...
| 19 | Bullet coverage                | Linear issue has ≥3 explicit bullets across its "Fix" + "Success criteria" sections | CRITICAL |
...
19. **Bullet coverage (multi-bullet issues only)** — fetch the Linear issue via `mcp__plugin_linear_linear__get_issue` and extract every explicit bullet from its **"Fix"** and **"Success criteria"** sections. Count only actual list items (`-`, `*`, numbered) in those two sections. Do **not** count bullets from "Problem," "Must not break," "Out of scope," or prose paragraphs.
    ...
    - **If the count is ≥ 3**: the PR body **must** contain an explicit Coverage table mapping each bullet to a commit or file. Fetch the PR body with `gh pr view --json body -q .body` (if the PR exists) or read `tmp/pr-body-s5u-<NUMBER>.md` if the worker staged it locally. Walk each bullet and confirm one of:
    ...
    - **Mandatory probe bullet**: the `Probes run:` section must include an explicit line of the form `"Bullet coverage: issue S5U-XXX has N bullets; #1 addressed by <path or sha>, #2 by <path or sha>, ... #K deferred to S5U-YYY (Linear confirms <state>)"`. A single "Bullet coverage: yes" line without per-bullet mapping is itself a **WARNING** that the reviewer should self-flag. The coverage probe is an auditable enumeration, not a checkbox.
    ...
    - **Retroactive test anchor**: this check, applied to S5U-594's PR #242, would enumerate 5 bullets (role, aria-modal, focus trap, focus restoration, initial focus) and flag the 3 that the worker dropped (focus trap, focus restoration, initial focus). See `tmp/plan-s5u-616.md` §4e for the walkthrough.
