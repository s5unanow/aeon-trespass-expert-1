...
| 20 | Must-refuse bullet coverage    | Issue labels ∈ {Bug, safety-gate, cross-system-review}                  | CRITICAL     |
| 21 | Semantically-equivalent threats| Issue labels ∈ {safety-gate, cross-system-review} OR (Bug AND diff adds a new gate/validator) | CRITICAL |
...
20. **Must-refuse bullet coverage (Bug / safety-gate / cross-system-review only)** — the Linear issue template (`.claude/prompts/linear-conventions.md` § "Must refuse") requires a **Must refuse** section for issues labeled `Bug`, `safety-gate`, or `cross-system-review`. Fetch the Linear issue via `mcp__plugin_linear_linear__get_issue` and check its labels:
    - **Trigger**: the issue has at least one of `Bug`, `safety-gate`, `cross-system-review`. Otherwise, skip this check.
    ...
    - **If the section contains `"None — this change has no untrusted input surface"`** or equivalent justified opt-out: accept it. Spot-check that the diff actually has no input surface before passing — if the diff touches filesystem paths, user-supplied IDs flowing into I/O, serialization, external process invocation, network boundaries, or auth, the "None" claim is false: **CRITICAL** — `"Must-refuse 'None' claim is false — diff contains {path} which introduces {untrusted-input-surface}"`.
    ...
    - **Mandatory probe bullet**: the `Probes run:` section must include an explicit line of the form `"Must-refuse coverage: issue S5U-XXX has N bullets; #1 implemented by <test or file:line>, #2 by <test or file:line>, ..."`. A single "Must-refuse: checked" line without per-bullet mapping is a **WARNING** the reviewer should self-flag; a determined reviewer enumerates each bullet. Non-triggering issues (Feature-only, etc.) note `"Must-refuse coverage: skipped — issue has no Bug/safety-gate/cross-system-review label"`.
21. **Semantically-equivalent threats enumeration (safety-gate / cross-system-review / Bug-with-new-check only)** — the Linear issue template (`.claude/prompts/linear-conventions.md` § "Semantically-equivalent threats") requires a **Semantically-equivalent threats** section on safety-gate changes. Fetch the issue via `mcp__plugin_linear_linear__get_issue` and check labels:
    - **Trigger**: the issue has `safety-gate` or `cross-system-review`, or is labeled `Bug` *and* the diff adds/tightens a validator, gate, or check (pre-commit hook, review gate, CI check, merge guard, validator function, config-schema constraint). Otherwise, skip this check.
    ...
    - **Mandatory probe bullet**: the `Probes run:` section must include `"Semantically-equivalent threats: issue S5U-XXX enumerates K vectors; verified #1 (<vector>) covered by <file>, #2 (<vector>) covered by <file>, ... ; audited <tool>'s --help / upstream docs for missed equivalences"`. A generic "Semantically-equivalent: checked" is a **WARNING** for reviewer self-discipline; the rule is the reviewer audits the upstream surface, not just the issue's enumeration. Non-triggering issues note `"Semantically-equivalent threats: skipped — issue has no safety-gate label and diff adds no new enforcement logic"`.
