...
25. **Admin-merge disclosure (S5U-671) — always run** — CLAUDE.md's NEVER list forbids merging with admin-bypass without a level-2 `## Admin-merge disclosure` heading in the PR body. This probe is the parallel to check #22 for the branch-protection surface: where #22 catches concealed local-hook bypass, #25 catches concealed branch-protection bypass. **Concealment grades as the stronger violation than the bypass itself** — an undisclosed admin-bypass is CRITICAL, a disclosed one is WARNING (audit-trail finding). The rule applies to PRs opened after S5U-671 lands; historical admin-merges (PRs #282–#288 from 2026-04-20) are not retroactively audited.
    - **Probe corpus**: the merge commit + PR body. Do **not** grep the diff. Do **not** rely on `gh pr merge --admin` strings appearing in commit messages (the admin-merge leaves no local trace — the worker invokes it after the push).
    - **Determine merge commit SHA**: `gh pr view <N> --json mergeCommit -q .mergeCommit.oid`. If the result is `null` or the command fails: probe is deferred. Probe bullet: `"Admin-merge disclosure: PR not yet merged; probe deferred to post-ship review"`. Do not file a finding — an unmerged PR cannot have an admin-merge yet.
    ...
    - **Fetch check-runs on merge commit**: `gh api repos/{owner}/{repo}/commits/{merge_sha}/check-runs --jq '.check_runs[] | {name, conclusion}'`. Cross-reference each required-context name against the `check_runs[].name` returned. For each required context, the matching check-run's `conclusion` determines bypass status:
        ...
        - `null` / `pending` / `queued` / (check-run absent entirely from response) — the merge happened before CI resolved. Not a hard admin-bypass, but a timing-class finding; record the context name with conclusion `pending`.
        - `neutral` / `skipped` — treat as `success` (the check voluntarily declined to report a pass/fail; not a bypass).

        **Stale-context carve-out (S5U-664 class)**: a required-context name whose check-run is **absent** from the response (never reported at all, as opposed to reported-and-failed) may be a stale-context mismatch — i.e., the workflow posts under a different name than branch protection requires. The reviewer should compare the required-context set to the check-runs `name` list and, if any required context has no matching check-run AND a sibling check-run exists with a different-but-related name AND that sibling has `conclusion=success`, this is the S5U-664 pattern. Worker disclosures citing this class are accepted while S5U-664 is unfixed; see grading rule below.
    - **Disclosure-heading check**: if any required context has a non-success / pending / absent status, grep the PR body for a level-2 heading:
        ```bash
    ...
    - **Grading**:
        - **No bypassed or pending required check** (all `success`/`neutral`/`skipped` AND no missing contexts): silent pass. Probe bullet: `"Admin-merge disclosure: merge commit <sha> — all required checks SUCCESS on merge commit; no admin-bypass detected"`.
        - **Bypassed check(s) (failure/cancelled/timed_out/action_required) AND disclosure heading absent or body empty**: **CRITICAL** — `"Admin-merge detected: required check(s) <names> were <conclusion> on merge commit <sha> but PR body lacks '## Admin-merge disclosure' heading (or section is empty). Concealment is a stronger violation than the bypass itself per CLAUDE.md NEVER list (S5U-671)."`.
        ...
        - **Bypassed check(s) AND disclosure present AND cites the S5U-664 stale-context carve-out** (text mentions "S5U-664", "stale-context", or equivalent, AND the bypassed check is absent-rather-than-failed, AND a sibling check-run with `conclusion=success` exists): call `mcp__plugin_linear_linear__get_issue(id="S5U-664")`. If state is NOT `Done`: **pass** — probe bullet `"Admin-merge disclosure: stale-context (S5U-664) class — disclosure cites S5U-664, sibling check <name> was SUCCESS on <sha>; carve-out valid while S5U-664 open"`. If state IS `Done`: **WARNING** — `"Admin-merge disclosure cites S5U-664 stale-context but S5U-664 is Done; carve-out no longer valid. Worker should have investigated the real cause of the context-name mismatch."`.
        - **Bypassed check(s) AND disclosure present AND names the bypassed check AND states a non-stale-context justification AND cites independent verification** (e.g., "reran `uv run pytest apps/pipeline/tests/unit/test_foo.py -k new_test` locally against merge SHA <sha>, output pasted below"): **WARNING** (audit-trail finding) — `"Admin-merge disclosed: required check <name> was <conclusion> on <sha>; disclosure present with justification '<excerpt>' and verification '<excerpt>'. Audit-trail WARNING per parallel to S5U-629."`. Reviewer should spot-check that the cited verification commands are plausible (e.g., the cited test exists in the diff).
        - **Pending-only check(s)** (no failure/cancelled, but a required check has `conclusion=null`/`pending` at merge) AND disclosure heading absent: **WARNING** — `"Merged while required check <name> was still pending on <sha>. Not a deliberate admin-bypass (CI may still be running), but PR merged before CI resolved — worker should have waited. If this was `--auto` merge triggered early, document in PR body."`. Grade WARNING, not CRITICAL, because this often happens via `--auto` race conditions rather than deliberate bypass.
        - **Pending-only check(s) AND disclosure heading present**: **NIT** — `"Merge-during-pending disclosed; no further action."`.
        - **Stale-context mismatch (required context absent, sibling present with success) AND disclosure heading absent**: **WARNING** — `"Likely S5U-664 stale-context mismatch (required context <name> never reported; sibling <alt_name> was SUCCESS on <sha>) but PR body lacks '## Admin-merge disclosure'. Worker likely admin-merged for the known S5U-664 class; disclosure still required per S5U-671."`. Severity is WARNING because the bypass was likely benign, but the concealment rule still applies; escalate to CRITICAL on repeat offenders.
    - **Mandatory probe bullet**: the `Probes run:` section must include one of:
        - `"Admin-merge disclosure: merge commit <sha> — all required checks SUCCESS on merge commit; no admin-bypass detected"`
        - `"Admin-merge disclosure: PR not yet merged; probe deferred to post-ship review"`
        - `"Admin-merge disclosure: merge commit <sha> — required check(s) <names> had conclusion <conclusion>; disclosure heading <present + cites (a)<check> (b)<justification> (c)<verification> — <grade> | absent — CRITICAL filed | bare N/A — CRITICAL filed>"`
        - `"Admin-merge disclosure: merge commit <sha> — required context <name> absent in check-runs, sibling <alt_name> = SUCCESS; S5U-664 stale-context class; S5U-664 state = <state>; <grade>"`
    - **This probe does NOT detect**: (a) an admin merging a PR where all required checks were `success` but who abused admin power for a different surface (e.g., overriding a CODEOWNERS review requirement) — the required-checks endpoint does not cover review requirements; (b) a pre-S5U-671 historical PR (rule applies to future PRs only); (c) a disclosure whose cited "independent verification" text is a fabrication — the reviewer can spot-check the cited commands but cannot replay arbitrary prose; (d) a merge that bypassed branch protection via `rulesets` overrides rather than the classic `required_status_checks` endpoint (this repo uses classic branch protection; rulesets are out of scope). Those are acknowledged residuals; the gate is worker honesty backed by the CLAUDE.md NEVER-list framing of concealment as the stronger violation, parallel to the S5U-629 hook-bypass probe's residuals.
