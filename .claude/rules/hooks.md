...
### SHA-resolution tripwire (S5U-624, extended S5U-651, S5U-924)
...
- **Do not attach a structural description to a SHA that does not contain it — the "mislabeled citation" (S5U-924).** A red-before block of the form `commit <sha> — terminator-class revert in <file> -> N cases fail` whose attached structural claim ("revert in `<file>`", "`<class>` mutation", "<helper> regex narrowed") is **contradicted by that SHA's actual diff** (`git show <sha> --name-only` doesn't touch `<file>`; the diff contains no such revert) is a mislabeled citation. It passes the existence + reachability check above precisely because the tripwire does not read the diff — so the safe-looking SHA hides a description of work that was never committed under it. The canonical failure: PR #404 cited `63a8866` (the *feature* commit, touching only the new corpus files) as a "terminator-class revert in `check_visual_gate_scope.py`"; the mutations had been done in a discarded scratch copy and never committed. Grading: **WARNING** when the underlying red-before work was clearly performed (e.g., independently reproducible in a scratch worktree) but the label is simply wrong; escalate toward **CRITICAL** when the mislabel looks like fabrication (no evidence the work was done, or the description is contrived to dress up an unrelated SHA). Preferred forms when the mutation was done in a discarded scratch copy and never committed: use form **(b)** the pasted local-run excerpt — `ran locally at <sha>^ with <mutation>; output: "<failure excerpt>"` — which makes the un-committed nature explicit, **not** a "revert in `<file>`" label pinned to an unrelated real SHA. If you want a SHA-backed citation instead, commit each mutation as a throwaway commit and cite **its** real SHA + failing test name + assertion excerpt (form (a)). The SHA-contents-vs-description check is a **reviewer spot-check on high-stakes diffs** (the deferred `git show <sha>` cross-check above), **not** a new hook or CI gate — we deliberately do not add a mechanical SHA-contents matcher; the gate remains worker honesty plus the reviewer's high-stakes spot-check.
